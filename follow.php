...
<body>
  <?php
	...
	$_SESSION['chats'] = '';
    $_SESSION['following'] = '';
	$_SESSION['following'] = $_GET['following'];
	  ...
	  <div class="col-sm-4" id="Third">
		...
		<div class="list-group">
			...
			<a href="follow.php?following=FOLLOWERS" class="list-group-item list-group-item-action">FOLLOWERS <?php
				...
				$conn->close();
			?></a>
			<button type='submit' name='save'>FOLLOW</button>
		</div>
	  </div>
	  <div class="col-sm-8" id="Second"> 
		  ...
		  <table class="table table-hover">
			...
			<?php
				...
				$tp=null;
				$tp=$_SESSION['following'];
				if($tp==FOLLOWING){
					$myquerry ="SELECT a.UserID, b.User, a.Name, a.Profile FROM users a inner join following b on a.UserID=b.FollowingUser WHERE b.User=1";
					$res = mysqli_query ($conn,$myquerry);
					if($res = $conn ->query($myquerry)){
						while($row = $res->fetch_assoc()){
							printf ("<img src='images/akanchha.jpg' class='img-rounded' alt='Cinque Terre' width='50' height='40'> <a href='search.php'><span class='notification-count-in-title'>%s %s </span></a><br><br><br>", $row["Name"], $row["Profile"]);
						}
					}
					else
						echo "errorr";
					...
					$myquerry ="SELECT a.UserID, b.FollowingUser, a.Name, a.Profile FROM users a inner join following b on a.UserID=b.User WHERE b.FollowingUser=1";
					$res = mysqli_query ($conn,$myquerry);
					if($res = $conn ->query($myquerry)){
						while($row = $res->fetch_assoc()){
							printf ("<img src='images/akanchha.jpg' class='img-rounded' alt='Cinque Terre' width='50' height='40'> <a href='search.php'><span class='notification-count-in-title'>%s %s </span></a><br><br><br>", $row["Name"], $row["Profile"]);
